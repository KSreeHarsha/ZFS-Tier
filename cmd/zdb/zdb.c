...
dump_znode_sa_xattr(sa_handle_t *hdl)
{
		...
		static uint64_t
get_filesize(objset_t *os, uint64_t object, void *data, size_t size)
{
...
dump_object(objset_t *os, uint64_t object, int verbosity, int *print_header)
{
		...
		if (dn->dn_type==19){
				...
				//printf("dnode object type is HH%d:",dn->dn_type);
				...
				//printf("caliculated asize is %d %d\r\n",asize,bsize);
				...
				if (verbosity >= 4) {
						...
						if (dn->dn_type==19)
								//printf("----%s-----",ZDB_OT_NAME(doi.doi_bonus_type));
				//				printf("-----Caliculated asize is %d\r\n",get_filesize(os, object,bonus, bsize));
...
void dmu_read_write(objset_t *os, uint64_t object,uint64_t offset,uint64_t size){
	...
	void *buf=umem_alloc(size, UMEM_NOFAIL);;
	...
	printf("Transaction group is %d\r\n",txg);
	
				if (assign_err == 0) {
						dmu_write(os,object, offset, size,buf, tx);
						dmu_tx_commit(tx);
						printf("Commiting Txg\r\n");
				} else {
						dmu_tx_abort(tx);
						printf("Aborting Txg\r\n");
				}
	umem_free(buf,size);	
		...
		static void
sync_object(objset_t *os, uint64_t object, int verbosity, int *print_header)
{
		dmu_buf_t *db = NULL;
		dmu_object_info_t doi;
		dnode_t *dn;
...
dump_dir(objset_t *os)
{
		...
		while ((error = dmu_object_next(os, &object, B_FALSE, 0)) == 0) {
				...
				sync_object(os, object, verbosity, &print_header);
				object_count++;
		}
...
zdb_read_block(char *thing, spa_t *spa)
{
		...
		{
				printf("printing the block pointer***********\r\n");	
				zdb_print_blkptr((blkptr_t *)(void *)
								((uintptr_t)buf + (uintptr_t)blkptr_offset), flags);
		}
		else if (flags & ZDB_FLAG_RAW)
		{	
				printf("dumping the block pointer***********\r\n");	
