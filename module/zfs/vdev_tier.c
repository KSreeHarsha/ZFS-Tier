...
typedef struct tier_map {
	int		mm_children;
	int		mm_replacing;
	int		mm_preferred;
	int		mm_root;
	tier_child_t	mm_child[1];
} tier_map_t;
 ...
 * why zfs_vdev_tier_switch_us is set to 10ms by default.  For non-
 ...
 *
 * For fast SSDs it may make sense to decrease zfs_vdev_tier_switch_us
 ...
 * use that to dynamically adjust the zfs_vdev_tier_switch_us time.
 */
int zfs_vdev_tier_switch_us = 10000;

static void
vdev_tier_map_free(zio_t *zio)
{
	tier_map_t *mm = zio->io_vsd;

	kmem_free(mm, offsetof(tier_map_t, mm_child[mm->mm_children]));
}

static const zio_vsd_ops_t vdev_tier_vsd_ops = {
	vdev_tier_map_free,
	zio_vsd_default_cksum_report
};

static int
vdev_tier_pending(vdev_t *vd)
{
	return (avl_numnodes(&vd->vdev_queue.vq_active_tree));
}

/*
 * Avoid inlining the function to keep vdev_tier_io_start(), which
 * is this functions only caller, as small as possible on the stack.
 */
noinline static tier_map_t *
vdev_tier_map_alloc(zio_t *zio)
{
	tier_map_t *mm = NULL;
	tier_child_t *mc;
	vdev_t *vd = zio->io_vd;
	...
	if (vd == NULL) {
		...
		for (c = 0; c < mm->mm_children; c++) {
			...
			mc->mc_pending = vdev_tier_pending(mc->mc_vd);
	...
	zio->io_vsd = mm;
	zio->io_vsd_ops = &vdev_tier_vsd_ops;
	return (mm);
}
...
static void
vdev_tier_child_done(zio_t *zio)
{
	tier_child_t *mc = zio->io_private;
...
static void
vdev_tier_scrub_done(zio_t *zio)
{
	tier_child_t *mc = zio->io_private;
 ...
 */
static int
vdev_tier_child_select(zio_t *zio)
{
	tier_map_t *mm = zio->io_vsd;
	tier_child_t *mc;
...
static int
vdev_tier_worst_error(tier_map_t *mm)
{
	int c, error[2] = { 0, 0 };

	for (c = 0; c < mm->mm_children; c++) {
		tier_child_t *mc = &mm->mm_child[c];
...
vdev_tier_io_done(zio_t *zio)
{
	...
	if (zio->io_type == ZIO_TYPE_WRITE) {
		...
		if (good_copies != mm->mm_children) {
			...
			if (good_copies == 0 || zio->io_vd == NULL)
				zio->io_error = vdev_tier_worst_error(mm);
		}
		return;
	}
	 ...
	 */
	/* XXPOLICY */
	if (good_copies == 0 && (c = vdev_tier_child_select(zio)) != -1) {
...
static void
vdev_tier_state_change(vdev_t *vd, int faulted, int degraded)
{
...
MODULE_PARM_DESC(zfs_vdev_tier_switch_us, "Switch tiers every N usecs");
#endif
